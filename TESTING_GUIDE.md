...
First, login through the UI at: https://wealth-dashboard-77.preview.emergentagent.com
...
curl -H "Authorization: Bearer $SESSION_TOKEN" \
  https://wealth-dashboard-77.preview.emergentagent.com/api/admin/database-stats
```
...
curl -X POST \
  ...
  https://wealth-dashboard-77.preview.emergentagent.com/api/admin/initialize-database
```
...
```bash
curl -H "Authorization: Bearer $SESSION_TOKEN" \
  https://wealth-dashboard-77.preview.emergentagent.com/api/admin/list-assets
```
...
```bash
curl -H "Authorization: Bearer $SESSION_TOKEN" \
  "https://wealth-dashboard-77.preview.emergentagent.com/api/data/search?q=Apple"
```

**Get Single Asset (AAPL):**
```bash
curl -H "Authorization: Bearer $SESSION_TOKEN" \
  https://wealth-dashboard-77.preview.emergentagent.com/api/data/asset/AAPL
```
...
curl -X POST \
  ...
  -H "Content-Type: application/json" \
  -d '["AAPL", "MSFT", "GOOGL"]' \
  https://wealth-dashboard-77.preview.emergentagent.com/api/data/assets/batch
```
...
curl -X POST \
  -H "Authorization: Bearer $SESSION_TOKEN" \
  "https://wealth-dashboard-77.preview.emergentagent.com/api/data/track?symbol=AAPL"
```

**Get Tracked Stocks:**
```bash
curl -H "Authorization: Bearer $SESSION_TOKEN" \
  https://wealth-dashboard-77.preview.emergentagent.com/api/data/tracked
```

### Option 2: Using Postman

1. **Import Collection:**
   - Base URL: `https://wealth-dashboard-77.preview.emergentagent.com/api`
...
1. **Open the app:** https://wealth-dashboard-77.preview.emergentagent.com
